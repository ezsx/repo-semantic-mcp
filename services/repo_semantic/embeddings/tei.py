...
class TeiProvider(EmbeddingProvider):
    ...
    def __init__(
        ...
        self._backend_name = backend_name.strip() or "tei_http"
        self._health_path = health_path.strip() if health_path else "/health"
    ...
    def _post_openai_embeddings_root(
        self,
        texts: list[str],
        prompt_name: str | None = None,
    ) -> list[list[float]]:
        """Отправить запрос в root `/embeddings` endpoint Infinity/OpenAI-compatible backend."""
    ...
    def _embed_with_split(
        ...
        try:
            return self._post_openai_embeddings_root(texts, prompt_name=prompt_name)
        except httpx.HTTPStatusError as exc:
            if exc.response.status_code != 413:
                raise
        except httpx.HTTPError:
    ...
    def healthcheck(self) -> None:
        ...
        response = httpx.get(f"{self._base_url}{self._health_path}", timeout=10)
        response.raise_for_status()

    def endpoint_label(self) -> str:
        """Вернуть HTTP endpoint TEI для диагностики."""
